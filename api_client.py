    ...
    def get_live_games(self) -> List[Dict]:
        ...
        try:
            ...
            live_games = [
                ...
                game.get('status', {}).get('short') == 2
            ]
            
            processed_games = []
            for game in live_games:
                home_team = game.get('teams', {}).get('home', {})
                away_team = game.get('teams', {}).get('visitors', {})
                
                processed_game = {
                    'id': self._safe_convert_id(game.get('id')),
                    'teams': {
                        'home': {
                            'id': self._safe_convert_id(home_team.get('id')),
                            'name': home_team.get('name', ''),
                        },
                        'away': {
                            'id': self._safe_convert_id(away_team.get('id')),
                    ...
                    'periods': {
                        'current': int(game.get('periods', {}).get('current', 0)),
                        'total': int(game.get('periods', {}).get('total', 4))
                    },
                    'status': {
